...
plotly <- fluidPage(
  ...
  uiOutput(outputId = "plotly_input"),

  selectInput("type_plotly","Plot type",choices = plot_choices),
  textInput("title_plotly",label = "Plot title",placeholder = "put your plot title"),
  plotlyOutput("plotly")
  ,
  br(),
  verbatimTextOutput("text_plotly")
)


gganimate <- fluidPage(

  uiOutput(outputId = "gganimate_input"),

  selectInput("type_gganimate","Plot type",choices = plot_choices),
  radioButtons("radio", h3("Radio buttons"),
               choices = list("transition_time" = 1, "transition_reveal" = 2,
                              "transition_events" = 3),selected = 1),
  textInput("title_gganimate",label = "Plot title",placeholder = "put your plot title"),
...
ui <- navbarPage(theme = shinytheme("united"),
                 ...
                 #   ,tags$head(tags$link(rel="icon", href="road2.jpg"))
)
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    if (nrow(data_a$annotations) > 0) {
      ...
      p <- p + geom_text(df2, mapping = aes(x,y, label = label, color = color), vjust = -1)
  ...
  final_code <- reactive({
    ...
    import_code <- if (input$data_use == 1) { "df <- ggplot2::mpg"} else {
      glue::glue("df <- read.csv('{input$file1$datapath}',
                                         header = '{input$header}',
                                         sep = '{input$sep}',
                                         quote = '{input$quote}',stringsAsFactors = F)")
    }
    ...
    background_code2 <- if (!is.null(input$file_background$datapath)) {
      paste0("\n\n",glue::glue("ggbackground(p, background = '{input$file_background$datapath}')"),"\n")
    } else {""}

    logo_code <- if (!is.null(input$file_logo$datapath)) {
      paste0("\n\n",glue::glue('logo <- png::readPNG({input$file_logo$datapath}) %>%
  ...
  output$plotly_input <- renderUI({

    div(
      selectInput("x_plotly", "X variable", choices = names(values$df)),
      selectInput("y_plotly", "Y variable", choices = names(values$df)),
      selectInput("fill_plotly", "Fill/Color", choices = c(names(values$df),"no_fill"),selected = "no_fill")
    )
  ...
  plotly_geom <- reactive({

    if(input$fill_plotly == "no_fill") {
      switch(input$type_plotly,
             Line = geom_line(),
             Bar = geom_col(),
             Box = geom_boxplot(),
             Scatter = geom_point())
      ...
      switch(input$type_plotly,
             Line = geom_line(aes(color= .data[[input$fill_plotly]])),
             Bar = geom_col(aes(fill= .data[[input$fill_plotly]])),
             Box = geom_boxplot(aes(color= .data[[input$fill_plotly]])),
             Scatter = geom_point(aes(color= .data[[input$fill_plotly]])))

    }
  })

  plotly_labs <- reactive({
    labs(x = input$x_plotly, y = input$y_plotly, title = input$title_plotly)
  })
  ...
  output$plotly <- renderPlotly({
    ...
    df <- values$df
    req(input$fill_plotly)

    a <- ggplot(df,aes(.data[[input$x_plotly]], .data[[input$y_plotly]])) +
      plotly_geom() +
      plotly_labs()
  ...
  final_code_plotly <- reactive({

    a <- paste0("a <- ggplot(df,aes(",input$x_plotly,", ",input$y_plotly,")")

    plotly_geom_code <- if(input$fill_plotly == "no_fill") {
      switch(input$type_plotly,
             Line = "geom_line()",
             Bar = 'geom_col()',
             Box = 'geom_boxplot()',
      ...
      switch(input$type_plotly,
             Line = paste0('geom_point(aes(color=',input$fill_plotly,'))'),
             Bar = paste0('geom_col(aes(fill=',input$fill_plotly,'))'),
             Box = paste0('geom_boxplot(aes(color= ',input$fill_plotly,'))'),
             Scatter = paste0('geom_point(aes(color= ',input$fill_plotly,'))'))
    }

    plotly_labs_code <- paste0('labs(x = ',"'",input$x_plotly,"'",',', ' y = ',"'",input$y_plotly,"'",',', 'title = ',"'",input$title_plotly,"'",')')


    b <- paste(a,"+", plotly_geom_code,"+",plotly_labs_code)

    paste(b, "ggplotly(a)", sep="\n")

  })

  output$text_plotly <- renderText({

    df <- values$df
    req(input$fill_plotly)

    paste(final_code_plotly())

  })

  # gganimate input ------------------------------------------------------------
  output$gganimate_input <- renderUI({

    div(
      selectInput("x_gganimate", "X variable", choices = names(values$df)),
      selectInput("y_gganimate", "Y variable", choices = names(values$df)),
      selectInput("fill_gganimate", "Fill/Color", choices = c(names(values$df),"no_fill"),selected = "no_fill"),
      selectInput("transition","transition variable",choices = c(names(values$df),""),selected = "")
    )

  })
  # gganimate render ------------------------------------------------------------

  gganim_geom <- reactive({

    if(input$fill_gganimate == "no_fill") {
      switch(input$type_gganimate,
             Line = geom_line(),
             Bar = geom_col(),
             Box = geom_boxplot(),
             Scatter = geom_point())
      ...
      switch(input$type_gganimate,
             Line = geom_line(aes(color= .data[[input$fill_gganimate]])),
             Bar = geom_bar(aes(fill= .data[[input$fill_gganimate]])),
             Box = geom_boxplot(aes(color= .data[[input$fill_gganimate]])),
             Scatter = geom_point(aes(color= .data[[input$fill_gganimate]])))

    }
  })

  gganim_labs <- reactive({
    labs(x = input$x_gganimate, y = input$y_gganimate, title = input$title_gganimate)
  })

  output$gganimate <- renderImage({

    df <- values$df
    req(input$transition)

    a <- ggplot(df,aes(.data[[input$x_gganimate]], .data[[input$y_gganimate]])) +
      gganim_geom() +
      gganim_labs()
    ...
    if (input$radio == 1) {
      ...
      p = a + transition_time(.data[[input$transition]]) + labs(subtitle = paste0(input$transition,"{frame_time}"))

    } else if (input$radio == 2) {

      p = a + transition_reveal(.data[[input$transition]]) + labs(subtitle = input$transition)
    } else {

      p = a + transition_events(.data[[input$transition]]) + labs(subtitle = paste0(input$transition,"{frame_time}"))
